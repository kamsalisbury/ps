...
# Stop the VMs specifying the verbose flag since the job log will show the data for troubleshooting and is low cost when configuring automation account diagnostic settings to log to a log analytics workspace
foreach ($vm in $targets) {
    Stop-AzVm -ResourceGroup $rg -Name $vm -WarningAction:Continue -ErrorAction:Continue -Verbose
}
